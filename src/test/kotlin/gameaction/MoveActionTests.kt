...
class MoveActionTests {
    ...
    fun test_pinned_check_works() {
        ...
        var moveAction = MoveAction(game.players.first(), Card(1), Direction.LEFT)
        assertFalse(moveAction.canTake(game))
        moveAction = MoveAction(game.players.first(), Card(1), Direction.RIGHT)
        assertFalse(moveAction.canTake(game))

        // Test pinned right
        game.board.movePlayer(0, 17)
        game.board.movePlayer(1, 15)
        moveAction = MoveAction(game.players.first(), Card(1), Direction.LEFT)
        assertFalse(moveAction.canTake(game))
        moveAction = MoveAction(game.players.first(), Card(1), Direction.RIGHT)
        assertFalse(moveAction.canTake(game))
    }
    ...
    fun test_when_adjacent_to_opponent_cannot_move() {
        ...
        var moveAction = MoveAction(game.players.first(), Card(2), Direction.RIGHT)
        assertFalse(moveAction.canTake(game))

        game.board.movePlayer(1, -2)

        moveAction = MoveAction(game.players.first(), Card(2), Direction.LEFT)
        assertFalse(moveAction.canTake(game))
    }
    ...
    fun test_can_move_if_wide_open() {
        ...
        val moveAction = MoveAction(game.players.first(), Card(3), Direction.RIGHT)
